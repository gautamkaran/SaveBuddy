...
const render = (links) => {
    let listItems = "";
    if (links.length === 0) {
        linkListEl.innerHTML = '<li style="text-align: center; color: var(--text-secondary); padding: 20px 0;">No links saved yet.</li>';
        return;
    }
...
const handleSaveTab = () => {
    if (typeof chrome !== 'undefined' && chrome.tabs) {
        chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
            if (tabs && tabs[0]) {
                myLinks.unshift({ id: generateUniqueId(), title: tabs[0].title || tabs[0].url, url: tabs[0].url });
                saveLinks();
                showMessage("Current tab saved!", "success");
            }
        });
    } else {
        showMessage("This feature is only available in a Chrome Extension.", "error");
    }
};
...
function handleEdit(event) {
    ...
    itemEl.querySelector('.cancel-edit-btn').addEventListener('click', () => {
        ...
        itemEl.querySelector('.delete-link-btn').addEventListener('click', handleDeleteLink);
    });
}
